    ...
    options: {
      ...
      customBodyRender: (value) => {
        if (value && value.length) {
          return (
            <>
              {_.map(value, (flag, index) => {
                const flagLower = _.lowerCase(flag);
                const color = _.includes(flagLower, 'warning')
                  ? '#ff9800'
                  : '#f44336';

                switch (flagLower) {
                  case _.includes(flagLower, 'temp'):
                    return (
                      <TempIcon
                        key={`iconTemp${index}`}
                        color={color}
                  ...
                  case _.includes(flagLower, 'humid'):
                    return (
                      <HumidIcon
                        key={`iconHumid${index}`}
                        color={color}
                  ...
                  case _.includes(flagLower, 'delay'):
                    return (
                      <DelayIcon
                        key={`iconDelay${index}`}
                        color={color}
                  ...
                  case _.includes(flagLower, 'recall'):
                    return (
                      <RecallIcon
...
export const getFormattedRow = (
  ...
  _.forEach(shipmentData, (shipment) => {
    ...
    editedShipment.shipment_flag = [];
    if (shipmentFlag && shipmentFlag.length > 0) {
...
export const svgIcon = (flagType, flag) => {
  ...
  if (flag) {
    const flagLower = _.lowerCase(flag);
    switch (flagLower) {
      case _.includes(flagLower, 'humidity'):
        if (_.includes(_.lowerCase(flagType), 'warning')) {
      ...
      case _.includes(flagLower, 'temperature'):
        if (_.includes(_.lowerCase(flagType), 'warning')) {
