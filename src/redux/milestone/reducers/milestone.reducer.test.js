...
describe('get repositories reducer', () => {
	...
	it('is successful', () => {
		...
		expect(reducer.default(initialState, { type: actions.GET_REPOSITORIES_SUCCESS, data })).toEqual({
			loading: false,
			loaded: true,
			error: null,
			repositories: [ 'repo-1' ],
...
describe('clear milestone headings reducer', () => {
	...
	it('failed', () => {
		expect(reducer.default(initialState, { type: actions.CLEAR_MILESTONES_HEADINGS_FAIL })).toEqual({
			loading: false,
			loaded: true,
			error: undefined,
			repositories: [],
